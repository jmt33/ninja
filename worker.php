...
use Process\Events\EventInterface;
use Process\Lib\Timer;
use Exception;
...
class Worker
{
    ...
    {
        // stop
        pcntl_signal(SIGINT, array('\Process\Worker', 'signalHandler'), false);
        // reload
        pcntl_signal(SIGUSR1, array('\Process\Worker', 'signalHandler'), false);
        // status
        pcntl_signal(SIGUSR2, array('\Process\Worker', 'signalHandler'), false);
        // ignore
    ...
    protected static function reinstallSignal()
    {
        ...
        self::$globalEvent->add(SIGINT, EventInterface::EV_SIGNAL, array('\Process\Worker', 'signalHandler'));
        // reinstall  reload signal handler
        self::$globalEvent->add(SIGUSR1, EventInterface::EV_SIGNAL, array('\Process\Worker', 'signalHandler'));
        // reinstall  status signal handler
        self::$globalEvent->add(SIGUSR2, EventInterface::EV_SIGNAL, array('\Process\Worker', 'signalHandler'));
    }
    ...
    public function run()
    {
        ...
        register_shutdown_function(array("\\Process\\Worker", 'checkErrors'));
        ...
        if (!self::$globalEvent) {
            $eventLoopClass    = "\\Process\\Events\\" . ucfirst(self::getEventLoopName());
